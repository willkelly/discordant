...
- ✅ No `npm install` needed for **local development**
- ✅ `package.json` contains Capacitor + minimal build deps for CI
- ✅ Vite, Svelte accessed via `npm:` specifiers locally (Deno handles it)
- ✅ `node_modules/` is auto-generated by Deno and gitignored
- ⚙️ CI/CD runs `npm ci` before build (for Vite plugins)
...
**Answer**: For Capacitor mobile builds AND build-time dependencies.

The package.json contains:
1. **Capacitor packages** - For iOS/Android mobile builds
2. **Build dependencies** - Vite plugins needed for `deno task build` in CI/CD
...
**Why build deps?** Vite's build process needs plugins physically installed.

**Local dev:** Deno accesses these via `npm:` specifiers - no `npm install` needed!

**CI/CD:** GitHub Actions runs `npm ci` before build to install Vite plugins.
